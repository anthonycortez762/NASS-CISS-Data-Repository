...
def compile_annual_nass_patient_files():
    ...
    for col in NASS_Constants.file_specific_values:
        col_name_avail[col] = {'event': True, 'vehicle': True, 'occupant': True}
    ...
    for file_ending in NASS_Constants.file_ending_map.keys():        
        ...
        for col in NASS_Constants.file_specific_values:
            col_lvl = NASS_Constants.file_lvl_map[file_ending]
            new_col_name = col + '_' + col_lvl
            if col in current_file.columns and col_name_avail[col][col_lvl]:
                current_file.rename(columns={col: new_col_name}, inplace=True)
                col_name_avail[col][col_lvl] = False
            elif col in current_file.columns and not col_name_avail[col][col_lvl]:
                current_file.drop(columns=col, inplace=True)
